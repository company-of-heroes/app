...
enum ClientMessage {
    ...
    Message { topic: String, data: serde_json::Value },
}
...
async fn broadcast_to_topic(
    ...
    let message = ServerMessage::Message { topic: topic.clone(), data };
    ...
    if let Some(client_ids) = topics.get(&topic) {
        ...
        Ok(format!("Message sent to {} subscribers of topic '{}'", sent_count, topic))
    } else {
        Ok(format!("No subscribers for topic '{}'", topic))
    }
}#[command]
async fn get_websocket_server_status(state: tauri::State<'_, WebSocketState>) -> Result<serde_json::Value, String> {
    ...
    let topic_info: HashMap<String, usize> = topics.iter()
                            ...
                            match client_msg {
                                ClientMessage::Subscribe { topic } => {
                                    // Add client to topic
                                    {
                                        ...
                                        topics.entry(topic.clone()).or_insert_with(HashSet::new).insert(client_id_clone2.clone());
                                        
                                        let mut subscriptions = client_subscriptions_clone.lock().unwrap();
                                    ...
                                    println!("Client {} subscribed to topic: {}", client_id_clone2, topic);
                                }
                                ClientMessage::Unsubscribe { topic } => {
                                    ...
                                    println!("Client {} unsubscribed from topic: {}", client_id_clone2, topic);
                                }
                                ClientMessage::Message { topic, data } => {
                                    // Emit received message to frontend
                                    let _ = window_clone.emit("websocket_message_received", serde_json::json!({
