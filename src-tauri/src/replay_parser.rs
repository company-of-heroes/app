...
impl BinaryReader {
    ...
    fn read_u8(&mut self) -> io::Result<u8> {
        if self.pos >= self.data.len() {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, format!("EOF reading u8 at pos {}/{}", self.pos, self.data.len())));
        }
    ...
    fn read_u16(&mut self) -> io::Result<u16> {
        if self.remaining() < 2 {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, format!("EOF reading u16 at pos {}/{}", self.pos, self.data.len())));
        }
    ...
    fn read_u32(&mut self) -> io::Result<u32> {
        if self.remaining() < 4 {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, format!("EOF reading u32 at pos {}/{}", self.pos, self.data.len())));
        }
    ...
    fn read_bytes(&mut self, count: usize) -> io::Result<&[u8]> {
        if self.remaining() < count {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, format!("EOF reading {} bytes at pos {}/{}", count, self.pos, self.data.len())));
        }
...
pub async fn parse_replay(path: String) -> Result<ParsedReplay, String> {
    ...
    // Parse Header
    result.replay_version = reader.read_u32().map_err(|e| format!("Header Version: {}", e))?;
    result.game_type = reader.read_ascii(8).map_err(|e| format!("Header GameType: {}", e))?;
    ...
    result.game_date = reader.read_utf16(date_len).map_err(|e| format!("Header Date: {}", e))?;
...
fn parse_chunky(reader: &mut BinaryReader, result: &mut ParsedReplay) -> io::Result<()> {
    if reader.remaining() < 12 { return Ok(()); }
    
    let header = reader.read_ascii(12)?;
    if header != "Relic Chunky" { return Ok(()); }

    reader.skip(4);
    if reader.read_u32()? != 3 { return Ok(()); }
...
fn parse_chunk(reader: &mut BinaryReader, result: &mut ParsedReplay) -> io::Result<bool> {
    if reader.remaining() < 8 { return Ok(false); }
    ...
    if chunk_type.starts_with("FOLD") {
        while reader.pos < start_pos + length as usize {
            if !parse_chunk(reader, result)? { break; }
        }
    } else if chunk_type == "DATASDSC" && version == 2004 { // 0x7d4
        reader.skip(4);
    ...
    } else if chunk_type == "DATABASE" && version == 11 { // 0xb
        reader.skip(16);
        ...
        // Skip versions
        let s_len = reader.read_u32()?; reader.read_ascii(s_len as usize)?;
        reader.skip(4);
        let s_len = reader.read_u32()?; reader.read_ascii(s_len as usize)?;
        reader.skip(8);
        ...
        if marker == 1 {
            ...
            if reader.read_u32()? > 0 {
                ...
                let (player_name, player_id) = if name_len > 0 {
                    (reader.read_utf16(name_len as usize)?, reader.read_u16()? as u32)
                } else {
            ...
            for _ in 0..bundle_count {
                if reader.pos >= tick_start + tick_len as usize { break; }
                ...
                if action_block_size > 0 && action_block_size < 65536 {
                    ...
                    if action_end_pos > reader.data.len() { break; }
...
fn parse_actions_in_block(reader: &mut BinaryReader, result: &mut ParsedReplay, tick: u32, end_pos: usize) -> io::Result<()> {
    while reader.pos + 2 <= end_pos {
        let action_len = reader.read_u16()?;
        if action_len == 0 || action_len > 1000 { break; }

        // Rewind to capture full action data including length bytes
        reader.pos -= 2;
        let action_start = reader.pos;
        
        if action_start + (action_len as usize) > end_pos { break; }
        ...
        let player_name = result.players.iter().find(|p| p.id == player_id).map(|p| p.name.clone()).unwrap_or_default();
        
        // Simple hex encoding
        let raw_hex = action_data.iter().map(|b| format!("{:02x}", b)).collect::<String>();
