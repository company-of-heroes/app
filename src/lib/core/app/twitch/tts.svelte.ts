...
export class TTS extends Module {
	 ...
	 * @type {boolean}
	 */
	enabled = $derived(app.settings.twitch.tts.enabled);
	 ...
	 * @type {string}
	 */
	channel = $derived.by(() => this.twitch.tokenInfo?.userName || '');
	 ...
	 * @type {Twitch}
	 */
	readonly twitch = $derived(app.modules.get('twitch'));
	...
	private async message(channel: string, user: string, message: string, msg: ChatMessage) {
		...
		if (provider === 'elevenlabs') await this.elabs(message, user);
	 ...
	 * @private
	 */
	private async elabs(message: string, user: string) {
		...
		const voice = app.settings.twitch.tts.elevenlabs.voiceName;
		// const voicesResponse = await this.twitch.elevenlabs?.client?.voices.getAll();
		const voiceId =
			app.modules
				.get('twitch')
				.tts?.elevenlabs?.voices?.find((v) => v.name === `[PERSONALITY] ${voice}`)?.voiceId ||
			app.modules.get('twitch').tts?.elevenlabs?.voices?.find((v) => v.name === voice)?.voiceId ||
			app.modules.get('twitch').tts?.elevenlabs?.voices?.find((v) => v.name === 'George')?.voiceId;
		...
		try {
			...
			while (true) {
				...
				if (done) {
					 ...
					 */
					try {
						this.elevenlabs?.client?.history.list().then(({ history }) => {
							this.elevenlabs?.client?.history.delete(history[0].historyItemId);
						});
					} catch (_) {
