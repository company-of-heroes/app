import { Replay, Tick } from './Replay.js';
...
export default class ReplayParser {
	...
	parse(fileName, actionDefinitions) {
		this.replay = new Replay(fileName, actionDefinitions);
		return this.parseReplay(this.replay, false);
	}

	parseReplay(replay, headerOnly = false) {
		this.replay = replay;
	...
	parseData() {
		...
		while (this.replay.replayStream.position < this.replay.replayStream.length) {
			if (this.replay.replayStream.readUInt32() === 1) {
				...
				tick = new Tick(this.replay.replayStream.readBytes(this.replay.replayStream.readUInt32()));
				this.parseTick(tick);
	...
	parseActions(tick, index) {
		...
		const bundleLength = tick.data.readUInt32BE(index + 9);
		...
		while (index + i + 2 <= tick.data.length) {
			...
			const L = tick.data.readInt16LE(index + i);
			if (L <= 0) {
			...
			const data = tick.data.slice(index + i, index + i + L);
